...
public class ItemRequestServiceImpl implements ItemRequestService {
    ...
    public List<ItemRequestDto> getAll() {
        return repository.findAllOrderByCreatedDesc().stream()
    ...
    @Override
    public ItemRequestWithItemsDto getById(long itemRequestId) {
        return repository.findById(itemRequestId)
                .map(req -> {
                    List<ItemDto> itemDtos = itemRepository.findAllByOwner_Id(req.getId())
                            .stream()
                            .map(ItemMapper::toItemDto)
                            .toList();
